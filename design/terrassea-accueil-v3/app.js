/* global document, window, IntersectionObserver, setInterval, clearInterval, setTimeout, clearTimeout, queueMicrotask, FormData, fetch, URL */
...
export class Page extends Accueil {
  constructor(root, settings = config) {
    ...
    this.texDefs = this.texDefs.map((t) => {
      ...
      return {
        ...
        size: 'cover',
        caption: `${t.ref} / ${url ? 'échantillon réel' : 'référence à raccorder'}`,
      }
    })
  ...
  configureMedia() {
    ...
    this.root.querySelectorAll('[data-form]').forEach((form) => {
      form.querySelector('.form-status').textContent = this.config.demo
        ? 'Démonstration locale : aucun envoi, aucune inscription réelle.'
        : ''
    })
  }
  ...
  async submitForm(event, type) {
    ...
    if (this.config.demo) {
      this.setState(type === 'project' ? { sent: true } : { sub: true })
      status.textContent =
        'Simulation uniquement — aucune donnée transmise et aucune inscription réelle.'
      return
    }
    const endpoint = this.config[`${type}Endpoint`]
    if (!endpoint) {
      status.textContent = 'Envoi non configuré. Votre saisie est conservée.'
      return
    }
    // Explicitly configured same-origin transport only; no default endpoint or analytics.
    const url = new URL(endpoint, window.location.origin)
    if (url.origin !== window.location.origin) {
      status.textContent = 'Endpoint non autorisé. Votre saisie est conservée.'
      return
    }
    this.setState(type === 'project' ? { sent: false } : { sub: false })
    ...
    try {
      ...
      status.textContent =
        'Envoi impossible. Votre saisie est conservée, veuillez réessayer.'
    } finally {
...
if (typeof document !== 'undefined' && document.getElementById('page'))
  new Page(document.getElementById('page'))
