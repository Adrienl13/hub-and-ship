...
export function ProductGallery({ product }: { product: Product }) {
  ...
  return (
    <section className="space-y-2" aria-label={`Galerie ${product.name}`}>
      <div className="group relative aspect-[4/3] overflow-hidden rounded-md bg-[color:var(--sand)] ring-1 ring-foreground/10 md:aspect-square">
        <img
          src={selectedImage}
        ...
        {hasMultipleImages && (
          <>
            <Button
              ...
              variant="outline"
              size="icon"
              className="absolute left-2 top-1/2 h-8 w-8 -translate-y-1/2 rounded-full border-white/35 bg-black/20 text-white opacity-0 backdrop-blur transition-opacity hover:bg-black/35 group-hover:opacity-100 focus-visible:opacity-100"
            ...
            <Button
              ...
              variant="outline"
              size="icon"
              className="absolute right-2 top-1/2 h-8 w-8 -translate-y-1/2 rounded-full border-white/35 bg-black/20 text-white opacity-0 backdrop-blur transition-opacity hover:bg-black/35 group-hover:opacity-100 focus-visible:opacity-100"
              onClick={goToNext}
          ...
          return (
            <button
              ...
              className={`relative aspect-square overflow-hidden rounded-sm bg-[color:var(--sand)] ring-1 transition ${
                selected ? "ring-2 ring-foreground" : "ring-foreground/10 hover:ring-foreground/35"
              }`}
            >
      ...
      {!hasMultipleImages && (
        <div className="rounded-sm border border-dashed border-[color:var(--sand-deep)] bg-card px-3 py-2 text-xs text-muted-foreground">
          Galerie fournisseur à compléter : vue matière, vue usage et détail empilage.
        </div>
      )}
    </section>
  );
