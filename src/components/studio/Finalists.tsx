...
export function Finalists({
  ...
  readonly visualRanking?: boolean
    ...
    <section aria-label="Vos finalistes" className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-[color:var(--ink-soft)]">
          {visualRanking
            ? 'À partir de ce que vous avez aimé et écarté, voici les pistes qui ressortent pour votre projet.'
      ...
      {finalists.length === 0 ? (
        <p className="rounded-md border border-[color:var(--sand-deep)] bg-[color:var(--paper)] p-4 text-sm">
          Aucun finaliste pour l&apos;instant : ajoutez des favoris avec «
          J&apos;aime ».
        </p>
      ) : (
        <ul
          className={`grid grid-cols-1 gap-4 sm:grid-cols-2 ${finalists.length > 2 ? 'lg:grid-cols-3' : ''}`}
              ...
              <div className="flex flex-1 flex-col gap-3 p-4">
                <div>
                  <div className="label-eyebrow text-[color:var(--ink-soft)]">
                    {[
                      seatKindLabel(product.studio.seatKind),
                      materialLabel(product.studio.material),
                    ]
                      .filter(Boolean)
                      .join(' · ') || 'Assise'}
                  </div>
      ...
      <div className="space-y-3">
        ...
        {moreCandidates.length > 0 && (
          <ul
            ...
            aria-label="Autres favoris à comparer"
            data-testid="more-finalists"
          >
            {moreCandidates.map((product) => (
              ...
              >
                ...
                <button
                  ...
                  title={
                    candidateAction(product.id) === 'compare'
                      ? 'Consulter sans ajouter aux finalistes'
                      : candidateAction(product.id) === 'replace'
                        ? 'Remplace le dernier finaliste'
                        : 'Ajoute aux finalistes'
                  }
