...
export function TableConfigurator({
  ...
  const [shapeId, setShapeId] = useState<TableShapeId>(() =>
    defaultShapeForProduct(product),
  )
  ...
  const selectedFoot =
    FOOT_FINISHES.find((finish) => finish.id === footFinishId) ??
    FOOT_FINISHES[0]!
      ...
      <div className="mb-4 flex items-start gap-2">
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            Le MOQ s'applique à la combinaison complète : format, plateau et
            finition pied.
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {FOOT_FINISHES.map((finish) => {
              ...
              return (
                ...
                >
                  <span
                    className="ring-foreground/15 h-4 w-4 rounded-full ring-1"
