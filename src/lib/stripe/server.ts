...
// (cf. wrangler config) and Node dev exposes them natively.

import Stripe from 'stripe'

let cachedClient: Stripe | null = null
let cachedCryptoProvider: ReturnType<
...
export function getStripe(): Stripe {
  ...
  if (!secretKey) {
    throw new Error(
      'STRIPE_SECRET_KEY is not set. Configure the secret or guard the ' +
        'call site with isStripeConfigured().',
    )
  }

  cachedClient = new Stripe(secretKey, {
    // `Stripe.createFetchHttpClient` returns an httpClient that relies on the
    // global `fetch` — required on Cloudflare Workers / workerd and harmless
    // on Node.
