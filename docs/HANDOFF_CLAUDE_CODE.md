...
| Factures/devis       | P1 | **FAIT** : recapitulatif imprimable `/account/reservations/{id}/document` (2026-06-07) ; **facture legale numerotee** (2026-06-08) — table `invoices` + sequence continue `invoice_number_seq`, RPC admin-only `issue_reservation_invoice` (numerotation atomique FAC-NNNNNN, snapshot immuable, refus de doublon), bouton admin "Facturer", facture imprimable `/account/reservations/{id}/facture/{invoiceId}` avec mentions legales FR completes. Reste : acompte/solde separes, avoir, envoi email de la facture. |
