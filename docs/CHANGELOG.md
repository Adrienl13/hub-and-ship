...
- **LOT 1 — Alignement des paliers de remise directe sur la grille officielle v2.** `CUSTOMER_QUANTITY_DISCOUNT_TIERS` (`src/lib/pricing/customer-discounts.ts`) passe de `50u→2% / 150u→6% / 300u→10%` à `100u→6% / 150u→10%`. Le code contredisait la grille v2 ; c'est désormais aligné.
