...
4. Sans exception : forme et dimensions positives finies obligatoires. Un type explicitement rattaché et vérifié permet la règle générale de forme/dimensions maximales. Une forme non couverte ou un dépassement est refusé. Les dimensions sont comparées grand côté/petit côté ; la rotation d'un rectangle est admise.
5. En l'absence de règle générale applicable au type, `compatibleTopShapes` peut être utilisé **seulement si la liste est non vide**. La fonction catalogue existante n'est appelée qu'après ces garde-fous.
