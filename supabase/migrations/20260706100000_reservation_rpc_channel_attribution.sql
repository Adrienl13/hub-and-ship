-- create_reservation_with_items v3 — canal + attribution (audit connecteurs).
--
-- Deux défauts critiques corrigés :
--   1. La v2 rejetait tout unit_price_ht ≠ products.base_price_ht : un client
--      revendeur/distributeur/grand_compte (prix canal servi par
--      get_catalogue_prices, LOT 4) ne pouvait PAS réserver — et si on levait
--      la validation naïvement, la ligne était re-tarifée au prix direct.
--      La v3 résout le prix canal du CALLER (même logique que
--      get_catalogue_prices v2 : override → marges actives → coefficient
--      statique, grand_compte = pire prix direct) et accepte le prix client
--      s'il correspond au prix canal OU au prix de base (fallback quand la
--      RPC de prix a échoué côté client — jamais inférieur au prix canal,
--      donc aucun risque de sous-facturation). Le prix validé est celui
--      stocké dans reservation_items.
--   2. La v2 ignorait silencieusement utm_source/utm_medium/utm_campaign/
--      partner_ref envoyés par le client (LOT 2) : l'attribution first-touch
--      n'atteignait jamais la base → aucune commission apporteur possible.
--      La v3 les insère.
