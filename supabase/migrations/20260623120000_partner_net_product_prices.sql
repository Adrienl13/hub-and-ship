-- Partner net product prices.
--
-- Public catalogue rows stay readable by anon, so partner net prices must NOT
-- live on public.products. This private companion table lets admins maintain
-- one net partner price per product while RLS keeps it hidden from anonymous
-- visitors and final-client share links.
...
comment on table public.product_partner_prices is
  'Private partner net prices per product. Never granted to anon; public catalogue keeps using products.base_price_ht.';
comment on column public.product_partner_prices.net_price_ht is
  'Net HT price shown only to admins and authenticated approved partners.';
  ...
  if v_product ? 'partner_net_price_ht' then
    ...
    if coalesce(v_partner_net_price, 0) > 0 then
      insert into public.product_partner_prices (
        product_id, net_price_ht, is_active
      ) values (
        v_product_id, v_partner_net_price, true
      )
