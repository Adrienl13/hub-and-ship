-- Migration 35 — visibilité « sur demande » (plateaux découpés sur mesure).
--
-- Un plateau découpé pour un projet a un prix négocié : l'admin le crée comme
-- produit avec visibility = 'on_request'. Il reste actif (réservable via un
-- lien de panier pré-rempli, fiche accessible en direct, noindex) mais
-- n'apparaît ni au catalogue public, ni dans le feed, le sitemap ou les
-- listes de plateaux du composeur.

alter table public.products
  add column if not exists visibility text not null default 'public'
    check (visibility in ('public', 'on_request'));

comment on column public.products.visibility is
  'public = listé au catalogue ; on_request = réservable uniquement par lien direct (projet sur mesure).';
...
select
  ...
  compatible_top_shapes, visibility
...
begin
  ...
  if v_is_create then
    ...
    update public.products set
      ...
      visibility = coalesce(nullif(v_product ->> 'visibility', ''), visibility),
