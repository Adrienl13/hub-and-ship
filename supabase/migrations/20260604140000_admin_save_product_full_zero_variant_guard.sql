-- DB safety net for the "product with 0 designs is invisible" defect.
--
-- A product with no product_variants row is filtered out of the public
-- catalogue (variants.length > 0 in src/lib/catalogue/db.ts) — it has no
-- orderable design. The admin editor now warns and forces is_active=false in
-- that case, but we enforce the same invariant at the end of the save RPC so
-- it holds for any caller and can never leave a product "actif mais
-- introuvable". Applied AFTER the variant delete+insert loops so a legitimate
-- "replace all designs" edit is not wrongly deactivated mid-transaction.
...
begin
  ...
  -- Invariant safety net: a product with no design is invisible on the public
  -- catalogue, so it must not remain active. Run after the variant loops so a
  -- legitimate "replace all designs" edit is not deactivated mid-transaction.
  if not exists (
