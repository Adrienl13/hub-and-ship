-- Migration 33b — composition de tables (piètement + plateau) et lounge.
--
-- 1. products.compatible_top_shapes : pour un piètement (table_base), formes
--    de plateau acceptées ('rectangular', 'round') ; vide = tous les plateaux.
-- 2. product_variants.min_order_units : minimum de commande PAR ACHETEUR
--    propre à un coloris (ex. un plateau d'une teinte spéciale ne se fabrique
--    qu'à partir de 40 pièces). NULL = règle du produit.
-- 3. Vue products_public : expose compatible_top_shapes (jamais les coûts).
-- 4. admin_save_product_full : persiste les deux nouveaux champs (corps
--    canonique de la migration 20260823110000 + ces ajouts).

alter table public.products
  add column if not exists compatible_top_shapes text[] not null default '{}';

comment on column public.products.compatible_top_shapes is
  'Piètements : formes de plateau compatibles (rectangular, round). Vide = tous.';

alter table public.product_variants
  add column if not exists min_order_units integer
    check (min_order_units is null or min_order_units > 0);

comment on column public.product_variants.min_order_units is
  'Minimum de commande par acheteur pour ce coloris (NULL = règle du produit).';
...
begin
  ...
  if v_is_create then
    insert into public.products (
      ...
      fire_rating, is_active, sort_order, table_shape,
      compatible_top_shapes
    )
    values (
      v_product_id,
      ...
      nullif(v_product ->> 'table_shape', ''),
      coalesce(
        array(select jsonb_array_elements_text(v_product -> 'compatible_top_shapes')),
        '{}'::text[]
      )
    );
  else
    update public.products set
      ...
      table_shape = case
        when v_product ? 'table_shape' then nullif(v_product ->> 'table_shape', '')
        else table_shape
      end,
      compatible_top_shapes = case
        when v_product ? 'compatible_top_shapes' then
          coalesce(
            array(select jsonb_array_elements_text(v_product -> 'compatible_top_shapes')),
            '{}'::text[]
          )
        else compatible_top_shapes
      end,
  ...
  loop
    ...
    insert into public.product_variants (
      id, product_id, name, image_url, gallery_urls, sort_order, min_order_units
    ) values (
      v_variant ->> 'id',
      v_product_id,
      v_variant ->> 'name',
      ...
      nullif(v_variant ->> 'min_order_units', '')::int
    )
    on conflict (id) do update set
      ...
      min_order_units = excluded.min_order_units;
  end loop;
  ...
  -- Garde-fou ré-intégré : un produit sans design est introuvable au catalogue
  -- public (et impossible à mettre en stock), il ne doit donc pas rester actif.
  if not exists (
