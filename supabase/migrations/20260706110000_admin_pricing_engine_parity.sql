-- Parité moteur de prix admin (audit régression pilotage) — v2 PRIVACY.
--
-- ⚠️ La v1 de ce fichier copiait admin_save_product_full d'AVANT le correctif
-- de confidentialité CODEX (le FOB partait dans public.products, lisible par
-- tous via le catalogue). La prod exécute la version POSTÉRIEURE
-- (20260702102000_private_product_pricing_inputs) : les coûts vivent dans
-- product_pricing_inputs (RLS admin + revoke anon), les colonnes publiques
-- sont purgées, et calculate/get_price lisent la table privée. Ce fichier
-- reflète CET état — idempotent : no-op en prod, protection identique sur
-- base neuve. Le prix d'achat ne doit JAMAIS être lisible publiquement.

-- Colonnes héritées sur products (nécessaires au scrub ci-dessous sur base
-- neuve ; en prod elles existent déjà, purgées à null).
