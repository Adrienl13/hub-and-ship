-- Correctif : « get_price: unknown or inactive product » au SAVE d'un produit.
--
-- Symptôme (prod 08/2026) : éditer le PRIX d'un produit inactif (ex. une
-- référence fraîchement importée, badge « Activer ») échoue avec
-- « Échec de l'enregistrement : get_price: unknown or inactive product
-- rope-rop-036 ». L'erreur remonte directement du RPC admin_save_product_full.
--
-- Cause : une version héritée de admin_save_product_full déployée en base
-- re-tamponnait le prix via get_price() en fin de sauvegarde. Or get_price()
-- REFUSE par conception tout produit inactif (`where id = ? and is_active`) —
-- donc plus aucune fiche inactive ne pouvait être enregistrée. Le save ne doit
-- JAMAIS dépendre de get_price : on écrit les données, le recalcul de prix est
-- une action admin explicite et séparée (bouton « Recalcul des prix »).
--
-- Ce fichier redéfinit admin_save_product_full avec le corps canonique propre
-- (aucun appel à get_price) et RÉ-INTÈGRE le garde-fou « produit sans design =
-- inactif » (perdu lors d'une refonte antérieure). create or replace →
-- il écrase la version stale déployée quel qu'en soit l'historique.
...
begin
  ...
  -- Après les boucles variantes pour ne pas désactiver un « remplacer tous les
  -- designs » légitime en cours de transaction.
  if not exists (
