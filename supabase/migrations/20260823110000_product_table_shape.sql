-- Forme des plateaux de table : rectangulaire (défaut) ou ronde.
--
-- Une table ronde se décrit par un diamètre, pas par longueur × largeur.
-- Convention de stockage (aucun schéma cassé) : pour une table ronde,
-- dim_length_cm = dim_width_cm = diamètre, et table_shape = 'round'.
-- L'affichage public devient « Ø 80 × H 75 cm ». Colonne NULL pour tout ce
-- qui n'est pas une table (chaises, fauteuils, bancs) et pour l'existant
-- (interprété comme rectangulaire).

alter table public.products
  add column if not exists table_shape text
    check (table_shape is null or table_shape in ('rectangular', 'round'));

comment on column public.products.table_shape is
  'Forme du plateau (tables uniquement) : rectangular | round. round ⇒ dim_length_cm = dim_width_cm = diamètre. NULL = rectangulaire / non applicable.';

-- Ré-émission d'admin_save_product_full : même corps canonique que la
-- migration 21 (aucun appel à get_price, garde-fou « sans design = inactif »)
-- + persistance de table_shape.
